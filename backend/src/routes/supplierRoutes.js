...
router.get("/", requireRole(["superusuario", "superadmin", "admin", "gerente"]), controller.listValidation, controller.listSuppliers);
router.get("/catalog/template", requireRole(["superusuario", "superadmin", "admin"]), controller.downloadSupplierCatalogTemplate);
router.get("/:id", requireRole(["superusuario", "superadmin", "admin", "gerente"]), controller.idValidation, controller.getSupplierDetail);
router.get("/:id/catalog", requireRole(["superusuario", "superadmin", "admin", "gerente"]), catalogController.listCatalogValidation, catalogController.listSupplierCatalog);
router.post("/:id/catalog/import/preview", requireRole(["superusuario", "superadmin", "admin"]), exportLimiter, uploadProductImportFile, catalogController.supplierIdValidation, catalogController.previewSupplierCatalogImport);
router.post("/:id/catalog/import/confirm", requireRole(["superusuario", "superadmin", "admin"]), exportLimiter, catalogController.confirmImportValidation, catalogController.confirmSupplierCatalogImport);
