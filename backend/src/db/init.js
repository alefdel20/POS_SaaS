...
// Migration 57 — healthcare.cardex_entries. Body copied verbatim from
// infra/postgres/57-healthcare-cardex-entries.sql. patient_id/pet_id are
// deliberately not NOT NULL individually — same criterion already applied to
// healthcare.pets.owner_id (migration 44) and healthcare.veterinary_encounters
// .owner_id (migration 48): a composite FK is simply not evaluated by
// Postgres when the column is NULL, so the CHECK (cardex_entries_subject_fk_
// check) is what actually enforces "exactly one of patient_id/pet_id".
async function ensureHealthcareCardexEntries(client) {
  await execQuery(
    client,
    `
  ...
  await run(client, [
    "CREATE UNIQUE INDEX IF NOT EXISTS uq_healthcare_cardex_entries_id_business ON healthcare.cardex_entries (id, business_id)",
    "CREATE INDEX IF NOT EXISTS idx_healthcare_cardex_entries_patient_date ON healthcare.cardex_entries (business_id, patient_id, event_date DESC)",
    "CREATE INDEX IF NOT EXISTS idx_healthcare_cardex_entries_pet_date ON healthcare.cardex_entries (business_id, pet_id, event_date DESC)",
    "CREATE INDEX IF NOT EXISTS idx_healthcare_cardex_entries_business_date ON healthcare.cardex_entries (business_id, event_date DESC)"
  ]);
