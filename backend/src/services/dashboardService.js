...
async function getSummary(actor) {
  ...
  const { rows } = await pool.query(
    `SELECT
       COALESCE((SELECT SUM(total) FROM sales WHERE sales.business_id = $1 AND sale_date = $2::date), 0) AS total_sales_today,
       COALESCE((SELECT SUM(total) FROM sales WHERE sales.business_id = $1 AND sale_date >= $2::date - INTERVAL '6 days'), 0) AS total_sales_week,
